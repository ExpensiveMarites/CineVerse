...
function GenrePages() {
    ...
    const { genres, openMovie, selectedMovieId } = useMovies();
    ...
    // Sorts any given list of movies based on the current filter
    const sortMovies = (list, criteria) => {
        return [...list].sort((a, b) => {
            ...
            return (b.popularity || 0) - (a.popularity || 0); // popular default
        });
    };
    ...
    useEffect(() => {
        const loadMovies = async () => {
            ...
            const data = await fetchMoviesByGenre(genreId, 1);

            // Sort initial fetch before setting state
    ...
    const loadMoreMovies = async () => {
        ...
        const data = await fetchMoviesByGenre(genreId, nextPage);
        ...
        setMovies((prev) => {
            ...
            // Apply current filter to ONLY the newly fetched movies
            const sortedNewChunk = sortMovies(newUnique, filter);

            // Append securely to the bottom so layout doesn't jump
    ...
    const handleMovieClick = (id) => openMovie(id);
    ...
    return (
        <section className="relative w-full min-h-screen bg-neutral-950 text-white flex flex-col lg:flex-row pt-16 md:pt-20 pb-10 lg:pb-16">
            ...
            {/* 🎬 GENRE SIDEBAR */}
            {/* <aside className="sticky top-16  w-full lg:w-64 xl:w-72 flex-shrink-0 bg-neutral-950/80 backdrop-blur-2xl border-b lg:border-b-0 lg:border-r border-white/5 lg:h-screen flex flex-col ">
                ...
                <div className="flex-1 min-h-0 min-w-0 overflow-x-auto lg:overflow-y-auto lg:overflow-x-hidden flex lg:flex-col flex-nowrap lg:flex-col gap-2 sm:gap-3 lg:gap-2 px-3 sm:px-4 lg:px-6 py-3 lg:py-6 snap-x lg:snap-none snap-mandatory scrollbar-hide">
                    {genres.map((genre) => {
                        ...
                        return (
                            <button
                                ...
                                className={`relative flex-shrink-0 lg:w-full px-4 sm:px-5 lg:px-4 py-2.5 sm:py-3 lg:py-3 rounded-full lg:rounded-xl text-xs sm:text-sm lg:text-base font-medium whitespace-nowrap lg:whitespace-normal snap-start flex items-center justify-between transition-all duration-300 ease-out focus-visible:ring-2 focus-visible:ring-red-500
                                    ${isActive
                ...
                <header className="relative w-full h-[25vh] md:h-[30vh] lg:h-[35vh] flex flex-col justify-start pt-8 lg:pt-16 px-4 sm:px-6 lg:px-12 overflow-hidden bg-neutral-950 flex-shrink-0">
                    ...
                    <div className="relative z-10 max-w-4xl lg:pt-10">
                        ...
                        <p className="text-sm sm:text-base md:text-lg text-neutral-300 mt-3 md:mt-4 max-w-2xl leading-relaxed drop-shadow-md">
                            Explore our curated collection of <span className="text-white font-medium">{selectedGenre?.name}</span> movies. Discover hidden gems and cinematic masterpieces tailored for you.
                        </p>

                        {/* FILTER DROPDOWN */}
                        <div className="mt-6 flex items-center gap-3">
                            <label htmlFor="sortFilter" className="text-sm font-medium text-neutral-400">
                                Sort by:
                            </label>
                                ...
                                {movies.map((movie) => {
                                    ...
                                    return (
                                        <article
                                            ...
                                            // animate-in allows newly added items at the bottom to smoothly fade in without jumping
                                    ...
                                    2.082-5.006z"
                                                        ...
                                                        <span className="bg-black/70 backdrop-blur-sm px-2 py-1 rounded-md border border-white/10">
                                                            {year}
                                                        </span>
                                                    </div>

                                                    <button tabIndex={-1} className="w-full py-2 sm:py-2.5 rounded-md text-xs sm:text-sm font-semibold bg-red-600 hover:bg-red-500 text-white shadow-[0_0_15px_rgba(220,38,38,0.4)] transition-colors duration-200">
                                                        View Details
                                                    </button>
                                                </div>
                                            </div>
